...
<p>For now, enquiries can be sent to <a href="mailto:{{ site.data.company.contact.email }}">{{ site.data.company.contact.email }}</a>.</p>
<p class="notice"><strong>Before public launch:</strong> confirm that this business email and the {{ site.data.company.brand.name }} domain are active. The value is centralised in <code>_data/company.yml</code> so it can be changed once for the whole site.</p>
</div>
